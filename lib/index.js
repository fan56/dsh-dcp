...
export class DcpEngine extends BasicCompactionEngine {
  ...
  /**
   * Handle one detected model switch. A switch within
   * `MODEL_SWITCH_MIN_ROUNDS` assistant messages of the session's last
   * committed compaction is ignored in every mode: nothing stale has
   * accumulated, so prompting (or compacting again) would only add noise
   * and rewrites. `notice` appends the suggest-`/dcp compact` row; `auto`
   * appends the in-progress row and marks the session for the next idle
   * boundary. The `auto` mode is an automatic compaction trigger, so the
   * `auto: false` master switch downgrades it to `notice`.
   */
