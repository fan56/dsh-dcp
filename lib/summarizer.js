...
/** First-line signals that a tool result is an error worth remembering. */
const ERROR_LINE = /(error|failed|failure|fatal|exception|enoent|eacces|eperm|denied|refused|not found|cannot |unable |exit code [1-9])/i
...
export function extractFacts(messages) {
  ...
  const rememberTodoLines = (text) => {
    ...
    for (const match of text.matchAll(TODO_LINE)) {
...
export function composeSummary(facts, options) {
  const { maxItems, maxItemChars, language, dedup, purgeErrors, sectionCap = maxItems, itemChars = maxItemChars, dropEmpty = false } = options
 ...
 * @param {(message: unknown) => number} estimateMessageLike - token estimator aligned with the host meter.
 * @returns {{ summary: { type: 'text', text: string }[], provider: string, model: string }}
 */
export function summarizeDeterministically(input, dcp, estimateMessageLike) {
